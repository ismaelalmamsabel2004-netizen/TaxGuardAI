...
export default function GeneradorFacturas() {
  ...
  // Cálculos matemáticos y limpieza de números para móvil
  ...
  // Objeto con todos los datos empaquetados para pasárselo al creador de PDFs
  const datosPDF = {
  ...
  // Función para guardar automáticamente el ingreso en la base de datos general
    ...
    try {
      ...
      const res = await fetch('/api/finances', {
        ...
        body: JSON.stringify({ 
          ...
          total: baseNum, // Guardamos la base como ingreso positivo
          empresaId: empresaId, 
          categoria: "Ventas", // Categoría automática
          isRecurrent: false, 
          iva: ivaSeleccionado 
        }) 
      });
          ...
          <main className="flex-1 p-4 pt-24 lg:pt-10 lg:p-10 overflow-y-auto w-full relative">
            <header className="flex flex-col lg:flex-row lg:justify-between lg:items-center mb-8 border-b border-slate-200 pb-6 gap-4">
              <div>
                ...
                <p className="text-sm font-medium text-slate-500 mt-1">Genera PDFs profesionales y regístralos automáticamente.</p>
              </div>
            ...
            {/* FORMULARIO DE FACTURACIÓN ESTILO MODERN UI */}
            <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden max-w-5xl">
              <div className="grid grid-cols-1 lg:grid-cols-2">
                ...
                {/* LADO DERECHO: CONCEPTOS E IMPORTES */}
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input type="text" placeholder="Ej: Consultoría web..." value={concepto} onChange={e => setConcepto(e.target.value)} className="w-full p-3 bg-white border border-slate-300 rounded-xl text-sm font-semibold text-slate-900 outline-none focus:ring-2 focus:ring-emerald-500/20 shadow-sm" />
                      </div>

                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Base Imponible (€)</label>
                          {/* 🚀 CORRECCIÓN DEL TECLADO NUMÉRICO BLINDADO */}
                          <input type="text" inputMode="decimal" placeholder="0.00" value={baseImponible} onChange={e => setBaseImponible(e.target.value)} className="w-full p-3 bg-white border border-slate-300 rounded-xl text-lg font-black text-slate-900 outline-none focus:ring-2 focus:ring-emerald-500/20 shadow-sm" />
                        </div>
                        <div>
